...
export const getDevices = (authToken) => {
    return (dispatch) => {
      ...
      axios
        .get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/deviceProfiles?pageSize=5&page=0`, {
          headers: {
  ...
  export const setDeviceAsDefault = (deviceid, authToken) => async (dispatch) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/deviceProfile/${deviceid}/default`, {
        method: 'POST',
        headers: {
