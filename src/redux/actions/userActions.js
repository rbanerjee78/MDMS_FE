...
export const fetchUsersSuccess = (users) => ({
  ...
  export const deleteUser = (userId) => async (dispatch) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/customer/${userId.id}`, {
        method: 'DELETE',
        headers: headers,
    ...
    return async (dispatch) => {
      try {
        ...
        const response = await axios.put(
          `${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/customer`,
          {
            id: userId,
            title: userTitle,
            city: userCity,
