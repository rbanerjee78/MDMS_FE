...
import { faCreditCard, faMoneyBillWave, faUserTie, faExclamationTriangle, faSearch, faEllipsisV, faEye, faEdit, faTrash } from '@fortawesome/free-solid-svg-icons';
...
export default function Prepaid() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`${process.env.REACT_APP_API_URL || 'http://localhost:5000'}/api/prepaidAccounts`);
    ...
    const lowBalance = data.filter(d => d.status === 'Low Balance').length;

    return (
        <div className='container my-3'>
            <div className="main-card py-3 px-3">
                <PageHeader title="Prepaid Accounts" subtitle="Account balances, top-ups, and low balance warnings" exportData={data} />
                
                <Row className="mb-4">
                    <Col md={4}>
                        <Card className="border-0 shadow-sm rounded-4 text-center p-3" style={{ background: 'linear-gradient(135deg, #d4fc79 0%, #96e6a1 100%)' }}>
                            <h6 className="text-muted text-uppercase fw-bold mb-2" style={{ fontSize: '12px' }}>Total Active Accounts</h6>
                            <h2 className="fw-bold text-success mb-0">{loading ? <Spinner animation="border" size="sm" /> : data.length}</h2>
                        </Card>
                    </Col>
                    <Col md={4}>
                        <Card className="border-0 shadow-sm rounded-4 text-center p-3" style={{ background: 'linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%)' }}>
                            <h6 className="text-muted text-uppercase fw-bold mb-2" style={{ fontSize: '12px' }}>Low Balance Alerts</h6>
                            <h2 className="fw-bold text-warning mb-0">{loading ? <Spinner animation="border" size="sm" /> : lowBalance}</h2>
                        </Card>
                    </Col>
                </Row>
                ...
                <div className='widget-card table-responsive shadow-lg rounded-4' style={{ minHeight: "50vh", backgroundColor: '#fff' }}>
                    ...
                    <table className="table table-hover align-middle mb-0">
                        <thead className="table-light text-muted" style={{ fontSize: '13px', textTransform: 'uppercase' }}>
                            <tr>
                                <th className="ps-3 border-0 rounded-start">Account ID</th>
                                <th className="border-0">Customer Name</th>
                                <th className="border-0">Current Balance</th>
                                <th className="border-0">Last Top-Up</th>
                                <th className="border-0">Top-Up Date</th>
                        ...
                        <tbody style={{ fontSize: '14px' }}>
                            {loading ? (
                                ...
                                const filteredData = data.filter(d => (d.customer || '').toLowerCase().includes(searchTerm.toLowerCase()) || (d.accountId || '').toLowerCase().includes(searchTerm.toLowerCase()));
                                ...
                                <tr key={row.id}>
                                    <td className="ps-3 fw-bold text-secondary"><FontAwesomeIcon icon={faCreditCard} className="me-2 text-primary"/>{row.accountId}</td>
                                    <td className="fw-bold text-dark"><FontAwesomeIcon icon={faUserTie} className="me-2 text-muted"/>{row.customer}</td>
                                    <td className="font-monospace text-primary fw-bold">{row.balance}</td>
                                    <td className="font-monospace text-success"><FontAwesomeIcon icon={faMoneyBillWave} className="me-1 text-success"/>{row.lastTopUp}</td>
                                    <td className="text-muted">{moment(row.lastTopUpDate).fromNow()}</td>
                                    <td className="text-center">
                                        {row.status === 'Low Balance' ? (
                                            <Badge bg="warning" text="dark" pill><FontAwesomeIcon icon={faExclamationTriangle} className="me-1"/>Low Balance</Badge>
                                        ) : (
